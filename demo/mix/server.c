...
server_cb(void *arg)
{
	...
	case RECV:
		...
		switch(used_send_policy){//some policies may have extra header
			default:{
				break;
			}
		}
	...
	case WAIT:
		...
		// The msg's pipe is the same as the pipe which received this msg
		// If pipe is not set, users should set it manually
...
server(const char *url)
{
	...
	nng_socket_set_int(sock,NNG_OPT_MIX_RECVPOLICY,NNG_RECVPOLICY_URGENT);
